...
const activeMenu = ref(route.path.startsWith('/knowledge') ? 'knowledge' : 'chat');

// 已废弃 isKnowledgeActive
...
watch(
  () => route.path,
  (newPath) => {
    activeMenu.value = newPath.startsWith('/knowledge') ? 'knowledge' : 'chat';
  },
  { immediate: true },
);
...
function handleMenuClick(menu: 'chat' | 'knowledge') {
  activeMenu.value = menu;
  if (menu === 'knowledge') {
    router.push('/knowledge');
  }
  else if (menu === 'chat') {
    // 直接跳转到聊天页面，而不是调用createSessionBtn
    router.push('/');
  }
}
</script>
...
<template>
  ...
  >
    <div class="aside-wrapper">
      <div v-if="!designStore.isCollapse" class="aside-header" style="padding-left: 12px;">
        <div class="flex items-center gap-8px hover:cursor-pointer" style="margin-left: 0;" @click="handleCreatChat">
          <el-image :src="logo" alt="logo" fit="cover" class="logo-img" />
          <span class="logo-text max-w-150px text-overflow">AI小智</span>
        </div>
      ...
      <div class="aside-body">
        ...
        <div class="menu-list">
          ...
          >
            <SvgIcon name="book-2-line" size="20" />
            知识库
          </div>
        </div>
